...
class LawyercateAction extends CommonAction {
    ...
    public function index() {
        $Lawyercate = D('Lawyercate');
        $list = $Lawyercate->fetchAll();
    ...
    public function create($parent_id=0) {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->baoSuccess('添加成功', U('lawyercate/index'));
            }
            $this->baoError('操作失败！');
        } else {
    ...
    public function update() {
        ...
        $obj->cleanCache();
        $this->baoSuccess('更新成功', U('lawyercate/index'));
    }
