...
class LawyerkowncateAction extends CommonAction {
    ...
    public function select(){
        ...
        $count = $Lawyerkowncate->where($map)->count(); // 查询满足要求的总记录数 
        $Page = new Page($count, 8); // 实例化分页类 传入总记录数和每页显示的记录数
        $pager = $Page->show(); // 分页显示输出
        $list = $Lawyerkowncate->where($map)->order(array('cate_id'=>'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $this->assign('list', $list); // 赋值数据集
    ...
    public function create($parent_id=0) {
        if ($this->isPost()) {
            ...
            $obj = D('Lawyerkowncate');
            $data['parent_id'] = $parent_id;
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->baoSuccess('添加成功', U('Lawyerkowncate/index'));
            }
            $this->baoError('操作失败！');
        } else {
    ...
    public function child($parent_id=0){
        $datas = D('Lawyerkowncate')->fetchAll();
        $str = '';
        ...
        if ($cate_id = (int) $cate_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->baoSuccess('操作成功', U('Lawyerkowncate/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    public function delete($cate_id = 0) {
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            $obj->delete($cate_id);
            $obj->cleanCache();
            $this->baoSuccess('删除成功！', U('Lawyerkowncate/index'));
        } else {
    ...
    public function update() {
        ...
        $obj->cleanCache();
        $this->baoSuccess('更新成功', U('Lawyerkowncate/index'));
    }
