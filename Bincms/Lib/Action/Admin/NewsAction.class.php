...
class NewsAction extends CommonAction {
    ...
    public function index() {
        ...
        $list = $News->where($map)->order(array('news_id'=>'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        ...
        $this->assign('cates', D('Newscate')->fetchAll());
        $this->display(); // 输出模板
    }
    ...
    public function edit($news_id = 0) {
        if ($news_id = (int) $news_id) {
            ...
            if ($this->isPost()) {
                ...
                $this->assign('parent_id',D('Newscate')->getParentsId($detail['cate_id']));
                $this->assign('cates', D('Newscate')->fetchAll());
                $this->display();
            }
        } else {
